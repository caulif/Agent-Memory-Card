...
fn sync_project_applies_memory_card_agent_targets_to_generated_artifacts() {
    ...
    let checkpoint = load_last_sync_checkpoint(temp.path())
        .expect("load checkpoint")
        .expect("sync checkpoint");
    assert_eq!(checkpoint.artifact_count, checkpoint.artifacts.len());
    assert!(
        checkpoint
            .artifacts
            .iter()
            .any(|artifact| artifact.path.ends_with("CLAUDE.md"))
    );
    assert!(
        checkpoint
            .memory_card_ids
            .contains(&"project:use-axios".to_string())
    );
    assert!(
        checkpoint
            .rollback_instructions
            .iter()
            .any(|step| step.contains("drift"))
    );

    let report = sync_project(temp.path()).expect("sync with verification");
    let verification = report.verification.expect("sync verification");
    assert_eq!(verification.status, "pass");
    assert_eq!(verification.rule_ci.failed, 0);
    assert!(
        verification
            .reload_prompt
            .contains("重新读取本项目的 AGENTS.md")
    );
}

#[test]
fn build_preview_reports_structured_artifact_diffs() {
    let temp = tempfile::tempdir().expect("tempdir");
    memory_card::add_memory_card(
        temp.path(),
        "project:review-boundary",
        "Keep Review Boundary",
        "Require human confirmation before overwriting generated agent instructions.",
        "preference",
        "project",
        vec!["codex".to_string()],
    )
    ...
    let report = build_project(temp.path(), true).expect("preview");
    let row = report
        .artifact_previews
        .iter()
        .find(|row| row.path.ends_with("AGENTS.md"))
        .expect("AGENTS.md preview row");

    assert_eq!(row.agent, "codex");
    assert_eq!(row.kind, "codex:instructions");
    assert_eq!(row.status, "create");
    assert!(row.current_hash.is_none());
    assert!(row.expected_hash.starts_with("sha256:"));
    assert!(
        row.diff_preview
            .iter()
            .any(|line| line.contains("Keep Review Boundary"))
    );
    assert!(
        row.diff_preview
            .iter()
            .any(|line| line.contains("Require human confirmation"))
    );
    assert_eq!(row.diff_truncated, false);
    assert!(
        row.diff_lines
            .iter()
            .any(|line| line.contains("Require human confirmation"))
...
#[test]
fn build_uses_distinct_agent_skill_names_for_non_ascii_memory_card_ids() {
    let temp = tempfile::tempdir().expect("tempdir");
    memory_card::add_memory_card(
        temp.path(),
        "global:按改动风险选择测试范围",
    ...
    .expect("add first memory_card");
    memory_card::add_memory_card(
        temp.path(),
        "global:优先用真实历史验证提炼逻辑",
        "优先用真实历史验证提炼逻辑",
        "优先使用真实历史验证提炼逻辑。",
        "procedure",
        "global",
    ...
    .expect("add second memory_card");

    sync_project(temp.path()).expect("sync");

    let skills_dir = temp.path().join(".agents").join("skills");
    let generated = fs::read_dir(&skills_dir)
        .expect("read skills")
        .filter_map(|entry| entry.ok())
        .filter_map(|entry| entry.file_name().into_string().ok())
        .filter(|name| name.starts_with("memory-card-"))
        .collect::<Vec<_>>();

    assert_eq!(generated.len(), 2);
    assert!(!skills_dir.join("global").exists());
}
...
#[test]
fn keep_artifact_drifts_accepts_current_manual_file_without_draft() {
    let temp = tempfile::tempdir().expect("tempdir");
    memory_card::add_memory_card(
        temp.path(),
        "project:codex-rule",
    ...
    sync_project(temp.path()).expect("initial sync");
    let path = temp.path().join("AGENTS.md");
    fs::write(&path, "manual edit to keep").expect("manual edit");

    let report = keep_artifact_drifts(temp.path()).expect("keep drift");

    assert_eq!(report.resolved, 1);
    assert_eq!(
        fs::read_to_string(&path).expect("agents"),
        "manual edit to keep"
    );
    assert_eq!(
...
#[test]
fn discard_artifact_drifts_restores_expected_generated_file() {
    let temp = tempfile::tempdir().expect("tempdir");
    memory_card::add_memory_card(
        temp.path(),
        "project:codex-rule",
    ...
    sync_project(temp.path()).expect("initial sync");
    let path = temp.path().join("AGENTS.md");
    let expected = fs::read_to_string(&path).expect("expected agents");
    fs::write(&path, "manual edit to discard").expect("manual edit");

    let report = discard_artifact_drifts(temp.path()).expect("discard drift");

    assert_eq!(report.resolved, 1);
    assert_eq!(fs::read_to_string(&path).expect("agents"), expected);
    assert_eq!(
        status_project(temp.path())
            .expect("status")
            .artifact_drift_count(),
...
#[test]
fn drift_resolution_can_keep_one_file_without_resolving_all_drifts() {
    let temp = tempfile::tempdir().expect("tempdir");
    memory_card::add_memory_card(
        temp.path(),
        "project:codex-rule",
    ...
    sync_project(temp.path()).expect("initial sync");
    let codex_path = temp.path().join("AGENTS.md");
    let claude_path = temp.path().join("CLAUDE.md");
    fs::write(&codex_path, "manual codex edit to keep").expect("codex edit");
    fs::write(&claude_path, "manual claude edit still drifted").expect("claude edit");

    let report =
        keep_artifact_drift_path(temp.path(), "AGENTS.md").expect("keep only AGENTS.md drift");

    assert_eq!(report.resolved, 1);
    assert_eq!(report.paths, vec!["AGENTS.md"]);
    assert_eq!(
        fs::read_to_string(&codex_path).expect("codex"),
        "manual codex edit to keep"
    );
    let status = status_project(temp.path()).expect("status");
    assert_eq!(status.artifact_drift_count(), 1);
}

#[test]
fn drift_resolution_can_import_one_file_without_importing_all_drifts() {
    let temp = tempfile::tempdir().expect("tempdir");
    memory_card::add_memory_card(
        temp.path(),
        "project:codex-rule",
    ...
    fs::write(
        ...
        "Always preview candidate evidence before approval.",
    )
    .expect("codex edit");
    fs::write(temp.path().join("CLAUDE.md"), "manual claude edit").expect("claude edit");

    let report =
        import_artifact_drift_path(temp.path(), "AGENTS.md").expect("import only AGENTS.md drift");
    ...
    assert!(drafts[0].body.contains("Always preview candidate evidence"));
    assert_eq!(
        status_project(temp.path())
            .expect("status")
            .artifact_drift_count(),
...
#[test]
fn import_artifact_drifts_covers_generated_agent_skill_artifacts() {
    let temp = tempfile::tempdir().expect("tempdir");
    ...
    let path = temp
        ...
        .join("agent-kernel-memory-card")
        .join("SKILL.md");
    let mut text = fs::read_to_string(&path).expect("read generated skill");
    text.push_str("\nAlways reconcile Memory Card drift before overwriting artifacts.\n");
    fs::write(&path, text).expect("manual generated skill edit");

    let report = import_artifact_drifts(temp.path()).expect("import generated skill drift");
    let drafts = draft::load_drafts(temp.path()).expect("drafts");

    assert_eq!(report.created, 1);
    assert_eq!(drafts.len(), 1);
    assert_eq!(drafts[0].targets, vec!["claude-code"]);
    assert!(
        drafts[0]
            .body
            .contains("Always reconcile Memory Card drift before overwriting artifacts.")
    );
    assert_eq!(
        status_project(temp.path())
            .expect("status")
    ...
    sync_project(temp.path()).expect("sync after importing generated skill drift");
}
