...
fn rules_artifact_file_name(agent_name: &str) -> &'static str {
    match agent_name {
        "cursor" => "agent-kernel.mdc",
        "cline" => "agent-kernel.md",
        _ => "agent-kernel.md",
    }
}
...
mod tests {
    ...
    #[test]
    fn build_writes_cline_rules_artifact_from_targeted_skilllet() {
        let temp = tempfile::tempdir().expect("tempdir");
        let mut project = config::default_project_config(temp.path());
        project.agents.get_mut("cline").expect("cline").enabled = true;
        ...
        skilllet::add_skilllet(
            ...
            "project:cline-rule",
            "Cline Rule",
            "Keep Cline task handoffs short and update memory files deliberately.",
            "constraint",
            "project",
            vec!["cline".to_string()],
        )
        ...
        let rule_path = temp.path().join(".clinerules").join("agent-kernel.md");
