# Agent-Kernel Iteration Backlog
...
Agent-Kernel is a local-first Skilllet evolution kernel for Claude Code and Codex. It turns local conversations, rules, successful workflows, and project conventions into reviewable, editable, assignable, and compilable Skilllets.
...
6. Assign approved Skilllets to Claude Code and/or Codex.
7. Compile and verify generated artifacts.

The storage principle is **file-native local store / no mandatory external DB**. Agent-Kernel should not require SQLite, Postgres, a vector database, or a background service, but it may and should maintain structured indexes, read-model caches, job files, and append-only audit logs under `.agent-kernel/` and `~/.agent-kernel/`.
...
- [x] Split heavy snapshot data into dedicated read endpoints: Drafts, Skilllets, Assignments, Quality, Build Preview.
- [x] Move Drafts, Skilllets, Catalog, Assignment, and Quality UI surfaces to page-level read models with `ProjectSnapshot` as fallback.
...
- [x] Make Skilllet fusion start a job and return immediately.
...
- [x] Prevent approving a Draft over an existing Skilllet without conflict review.
- [x] Validate Draft and Skilllet editable fields: non-empty title/body, known kind/scope, known agent targets.
...
- [x] Skilllet editor can update title, brief, body, kind, scope, tags, and language.
...
- [ ] Add conflict notes to Draft / Skilllet fusion outputs.

## P3 - AI Fusion And Recommendation

- [ ] Replace simple Skilllet concatenation with engine-backed semantic fusion.
...
- [ ] Add project recommendation: recommend global Skilllets and Skills after project scan/import.
...
- [ ] Add package registry metadata for Skilllet packs.
- [ ] Add import/export for global Skilllet library bundles.
