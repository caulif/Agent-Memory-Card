...
虽然用户入口仍然可以是 `npx agent-kernel`，但核心实现建议使用 Rust，而不是 TypeScript。
...
agent-kernel/
  ...
  npm/
    agent-kernel/           # npx wrapper，下载/调用平台二进制
```

Rust 适合这里的原因：

- 文件治理和 diff 需要稳定、快、可预测。
- 单二进制适合通过 npm、Homebrew、Cargo、GitHub Releases 多渠道分发。
...
```md
<!-- Generated by Agent-Kernel. Do not edit directly. Run `npx agent-kernel import` to ingest manual changes. -->
```
...
```bash
npx agent-kernel search react
npx agent-kernel install @frontend/react-best-practices
npx agent-kernel install @backend/fastapi-clean-architecture
npx agent-kernel update
npx agent-kernel audit-registry
```
...
```bash
npx agent-kernel import --scan-home --project .
npx agent-kernel ui
```
...
```bash
cat session.md | npx agent-kernel extract --project .
```
...
1. 规则层：同一 conflict key 下存在否定关系，例如 `fetch` vs `axios`、`npm` vs `pnpm`。
...
- 核心概念：`skilllet`
- 核心实现：Rust binary
- 分发方式：npm wrapper + GitHub Releases，后续支持 Homebrew / Cargo install
- 子命令：`gc`、`extract`、`review`、`export`、`attach`、`ui`

命令草案：

```bash
npx agent-kernel init
npx agent-kernel scan
npx agent-kernel extract ./chat.md
npx agent-kernel gc --dry-run
npx agent-kernel review
npx agent-kernel build
npx agent-kernel import ./CLAUDE.md
npx agent-kernel test
...
```bash
npx agent-kernel import ./AGENTS.md
npx agent-kernel gc --interactive
npx agent-kernel build --to codex --preview
```
